...
def test_streamlit_pages_have_unique_url_paths_and_meet_setup_default():
    ...
    assert url_paths == ["meet-dashboard", "meet-setup", "athletes", "athlete-profile", "team-progress", "coach-analytics", "configure-race", "live-timing", "results", "school-branding", "live-race", "coach-sign-in", "race-day-timer", "race-day-roster"]
...
def test_callable_page_switches_use_registered_page_objects_not_file_paths():
    for relative_path in ["pages/meet_dashboard.py", "pages/meet_setup.py", "pages/live_timing.py"]:
