...
from split_tracker.timer_mode import TimerRaceOption, build_timer_options
...
def _select_station(option: TimerRaceOption, checkpoint_number: int) -> None:
    ...
    st.session_state.timer_name = f"{checkpoint.label} • {identity.email}"
...
def render() -> None:
    ...
    st.caption("Only races and checkpoints needed for live split capture are shown.")
